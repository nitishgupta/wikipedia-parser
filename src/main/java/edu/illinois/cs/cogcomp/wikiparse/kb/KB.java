...
public class KB {
	...
	public static Map<String, String> widMidMap_WrittenPages = new HashMap<>();
	...
	static {
		...
		makeNameAliasMap();
		System.out.println("[#] Making wid->mid for mids in mid.names.wiki_id that are legitimate in Wikipedia");
		foundInWiki();
		System.out.println("[#] Making wid->mid for pages with text in docs_links of : " + wiki_kb_docsDir);
		writtenWikiPages();
	}
	...
	private static void writtenWikiPages() {
		...
		for (String wid : new File(wiki_kb_docsDir).list()) {
			widMidMap_WrittenPages.put(wid, widMidMap.get(wid));
		}

		Set<String> notParsed = new HashSet<String>(widMidMap_FoundInWiki.keySet());
		notParsed.removeAll(widMidMap_WrittenPages.keySet());

		StringBuffer notparsed = new StringBuffer();
		for (String wid : notParsed){
			notparsed.append(wid + "\n");
		}
		FileUtils.writeStringToFile("/save/ngupta19/wikipedia/wiki_kb/notparsed", notparsed.toString());
	}


	public static void main(String [] args) {
		System.out.println(KB.widMidMap_WrittenPages.size());
	}
